...
function showResult(result){const resultScreen=document.getElementById('screen-result');resultScreen.classList.remove('result-enter');show('screen-result');document.getElementById('resultName').textContent=result.name;document.getElementById('resultTagline').textContent=result.tagline;document.getElementById('resultReason').textContent=result.reason;document.getElementById('heroType').textContent=getType(state.result.key,state.answers);const stats=document.getElementById('stats');stats.innerHTML='';Object.entries(result.stats).forEach(([label,value])=>{const row=document.createElement('div');row.className='stat-row';row.innerHTML=`<span>${label}</span><div class="stat-bar"><div class="stat-fill" style="width:${value*20}%"></div></div><span class="stat-value">${'★'.repeat(value)}</span>`;stats.appendChild(row)});const second=state.result.sorted.find(([k])=>k!==state.result.key);document.getElementById('runnerUp').textContent=second?`次点装備：${services[second[0]].name}　― 旅の目的によっては こちらも有力じゃ。`:'';requestAnimationFrame(()=>resultScreen.classList.add('result-enter'));audioManager.playResultSequence();document.getElementById('warpBtn').onclick=()=>{sfx('gate');game.classList.add('flash');setTimeout(()=>{window.location.href=result.url},720)}}
